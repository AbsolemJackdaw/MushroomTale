...
public class Spirit extends Enemy {
	...
	// attack pattern
	private final int[] steps = { 0, 1, 0, 1, 2, 1, 0, 2, 1, 2 };
	...
	private final DarkEnergy[] shield;
	...
	public Spirit(TileMap tm, Player p, ArrayList<Enemy> enemies,
		...
		try {
			final BufferedImage spritesheet = ImageIO.read(getClass()
					.getResourceAsStream("/Sprites/Enemies/Spirit.gif"));
	...
	public void update() {
		...
		if (finalAttack) {
			...
			if ((stepCount >= 90) && ((stepCount % 30) == 0)) {
				final DarkEnergy de = new DarkEnergy(tileMap);
				de.setPosition(x, y);
				de.setVector(3 * Math.sin(stepCount / 32),
						3 * Math.cos(stepCount / 32));
		...
		// shockwave
			...
			if (y >= (tileMap.getHeight() - 30))
				dy = 0;
			if ((stepCount > 60) && (stepCount < 120) && ((stepCount % 5) == 0)
					&& (dy == 0)) {
