...
public class HUD {
	...
	public HUD(Player p) {
		...
		try {
			img = ImageIO.read(getClass()
					.getResourceAsStream("/player/hud.png"));
	...
	public void draw(Graphics2D g) {
		...
		for (int i = 0; i < player.getMaxHealth(); i++)
			g.drawImage(img.getSubimage(30, 0, 30, 30), i > 5 ? -20
					+ (30 * (i - 5)) : 10 + (30 * i), i > 5 ? 35 : 10, null);

		// hearts
		for (int i = 0; i < player.getHealth(); i++)
			g.drawImage(img.getSubimage(00, 0, 30, 30), i > 5 ? -20
					+ (30 * (i - 5)) : 10 + (30 * i), i > 5 ? 35 : 10, null);
		...
		if (((30 - (int) c) >= 0) && ((40 - (int) c) >= 0))
			g.drawImage(img.getSubimage(90, 30 - (int) c, 30, 30), 280,
					40 - (int) c, null);
